	...
	if (isset($_POST['btnSend'])){
		...
		if(mysqli_num_rows($result)==0){
				echo("<SCRIPT LANGUAGE='JavaScript'>
					...
					window.location.href='petredeem.php'
					</SCRIPT>");
				exit();
		}else{
			...
			//Check first if stated litres are available at service station
				...
				if ($balla < $lit){
				    echo("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('The service station petrol balance is insufficient. Redemption saving failed.');
					window.location.href='petredeem.php';
					</SCRIPT>");
					exit();
				}
				
			//Check first if stated litres are there also if petrol account exists
				...
				if(mysqli_num_rows($result)==1){
					...
					if($stat!='Active'){
						echo("<SCRIPT LANGUAGE='JavaScript'>
						window.alert('Your petrol account is currently blocked. Contact Total sales team for assistance.');
						window.location.href='petredeem.php';
						</SCRIPT>");
						exit();
					}
					...
					IF ($balance < $lit){
				    echo("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('Your petrol fuel balance is insufficient. Redemption saving failed.');
					window.location.href='petredeem.php';
					</SCRIPT>");
					exit();
					}
					...
					echo("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('You have no any petrol account with total fuel services.')
			...
			session_destroy();
			echo("<SCRIPT LANGUAGE='JavaScript'>
				window.alert('Your fuel redemption is successfull.')
...
<html class="no-js" lang="">
	...
	<div class="container" style="background-color:rgba(0,0,0,0.8);padding: 5px 25px; margin-top:160px;-moz-border-radius:15px;-webkit-border-radius:15px;height:600px;">
	<h2 style="text-align: center;color: #cc9900;font-weight:bold;font-size:20pt;">Petrotrade Electronic Coupon System Petrol Redemption</h2>
	<hr>
			<div class="col-lg-4">
				<img src="images\logo.png" style="align:center;-moz-border-radius:15px;-webkit-border-radius:15px;"/>
	        </div>
		<form method="POST" action="petredeem.php" autocomplete="off" name="myForm">
			<div class="col-lg-4">
				<div class="form-group ">
						<h3 style="text-align: center;color:#cc9900;font-size:12pt;">Petrol Station Code Number</h3>
                        <input type="number" name="txtStation" size=10 class="form-control">
                </div>
				<div class="form-group ">
						<h3 style="text-align: center;color:#cc9900;font-size:12pt;">Petrol Volume In Litres</h3>
