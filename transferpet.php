		...
		//Check first if stated litres are there also if diesel account exists
			...
			$result=mysqli_query($myConn,$sql);//will execute the query into the database
				...
				if(mysqli_num_rows($result)==1){
					...
					IF ($balance < $lit){
				    echo("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('Your petrol fuel balance is insufficient. Transfer transection failed.');
					window.location.href='transferpet.php';
					</SCRIPT>");
					exit();
					}
					$lftBal=$balance-$lit;
					...
					if(mysqli_num_rows($result)==0){
						...
						if (mysqli_num_rows($result)==1){
							...
							if($stat!='Active'){
								echo("<SCRIPT LANGUAGE='JavaScript'>
								window.alert('The recepient petrol account is currently blocked. Contact Total sales team for assistance.');
								window.location.href='home.php';
								</SCRIPT>");
								exit();
							}
...
<html class="no-js" lang="">
	...
	<h2 style="text-align: center;color: #cc9900;font-weight:bold;font-size:20pt;">Petrol Transfers</h2>
	<hr>
			<div class="col-lg-4">
		...
		<form method="POST" action="transferpet.php" autocomplete="off" name="myForm">
			<div class="col-lg-4">
				<div class="form-group ">
						<h3 style="text-align: center;color:#cc9900;font-size:12pt;">Phone Number To Receive Petrol</h3>
